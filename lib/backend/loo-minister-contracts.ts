...
export const looMinisterSecurityIdentitySchema = z.object({
  ...
  if (hasExchange !== hasCurrency) {
    context.addIssue({
      ...
      message: "Security identity must keep exchange and currency together when either is available."
    });
  }
});
...
export const looMinisterFactSchema = z.object({
  ...
  source: z.enum(["portfolio-data", "quote-cache", "fx-cache", "user-input", "analysis-cache", "system"]).default("portfolio-data")
});

export const looMinisterSuggestedActionSchema = z.object({
  id: z.string().trim().min(1).max(80),
  ...
  if (
    ["create-draft", "update-preferences", "refresh-data", "run-analysis"].includes(value.actionType) &&
    !value.requiresConfirmation
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["requiresConfirmation"],
      message: "Actions that create, update, refresh, or run analysis must require explicit user confirmation."
    });
  }
});

export const looMinisterPageContextSchema = z.object({
  ...
  dataFreshness: z.object({
    ...
    chartFreshness: z.enum(["fresh", "stale", "fallback", "reference", "unknown"]).default("unknown"),
    sourceMode: z.enum(["local", "cached-external", "live-external", "reference"]).default("local")
  }).default({
    portfolioAsOf: null,
    quotesAsOf: null,
    fxAsOf: null,
  ...
  allowedActions: z.array(looMinisterSuggestedActionSchema).max(12).default([])
}).superRefine((value, context) => {
  ...
  if (value.page === "security-detail" && !value.subject.security) {
    context.addIssue({
      ...
      message: "Security detail minister context requires resolved security identity."
    });
  }

  if (value.dataFreshness.sourceMode === "local" && value.dataFreshness.chartFreshness === "reference") {
    context.addIssue({
      code: z.ZodIssueCode.custom,
...
export const looMinisterQuestionRequestSchema = z.object({
  ...
  if (value.includeExternalResearch) {
    context.addIssue({
      ...
      message: "Live external research is not available for Loo Minister answers until worker/cache policy is enabled."
    });
  }
});

export const looMinisterAnswerResultSchema = z.object({
  version: z.literal(LOO_MINISTER_VERSION),
  ...
  suggestedActions: z.array(looMinisterSuggestedActionSchema).max(8).default([]),
  sources: z.array(z.object({
    title: z.string().trim().min(1).max(200),
    sourceType: z.enum(["page-context", "portfolio-data", "quote-cache", "fx-cache", "analysis-cache", "manual"]),
...
export type LooMinisterSecurityIdentity = z.infer<typeof looMinisterSecurityIdentitySchema>;
export type LooMinisterPageContext = z.infer<typeof looMinisterPageContextSchema>;
export type LooMinisterQuestionRequest = z.infer<typeof looMinisterQuestionRequestSchema>;
export type LooMinisterAnswerResult = z.infer<typeof looMinisterAnswerResultSchema>;
export type LooMinisterSuggestedAction = z.infer<typeof looMinisterSuggestedActionSchema>;
