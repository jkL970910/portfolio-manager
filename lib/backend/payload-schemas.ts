...
const recommendationConstraintsSchema = z
  .object({
    excludedSymbols: symbolListSchema.default([]),
    preferredSymbols: symbolListSchema.default([]),
    excludedSecurities: z
      .array(securityConstraintIdentitySchema)
      .max(50)
      .default([]),
    preferredSecurities: z
      .array(securityConstraintIdentitySchema)
      .max(50)
      .default([]),
    assetClassBands: z
    ...
    avoidAccountTypes: z
      ...
      .max(4)
      .default([]),
    preferredAccountTypes: z
      .array(z.enum(["TFSA", "RRSP", "FHSA", "Taxable"]))
      .max(4)
      .default([]),
    allowedSecurityTypes: z
...
export const preferenceProfileInputSchema = z
  .object({
    ...
    accountFundingPriority: z
      ...
      .refine(
        ...
        "Account priorities must be unique.",
      ),
  ...
  .superRefine((value, context) => {
    const total = value.targetAllocation.reduce(
      (sum, target) => sum + target.targetPct,
      0,
    );
    if (total !== 100) {
      context.addIssue({
    ...
    if (value.mode === "loo-zh") {
      if (!value.gender) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["gender"],
          message: "Gender is required for Loo citizenship registration.",
        });
      }
      if (!value.birthDate) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["birthDate"],
          message: "Birth date is required for Loo citizenship registration.",
        });
      }
      if (value.acceptLooTerms !== true) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["acceptLooTerms"],
          message: "Loo terms must be accepted.",
        });
      }
    }
  });
...
export const citizenOverrideInputSchema = z
  .object({
    rank: z
      .enum(["lowly-ox", "base-loo", "citizen", "general", "emperor"])
      .nullable()
      .optional(),
    addressTier: z
      .enum(["cowshed", "suburbs", "city", "palace-gate", "bedchamber"])
      .nullable()
      .optional(),
  ...
  .refine(
    (value) =>
      ...
      value.addressTier !== undefined ||
      value.idCode !== undefined,
...
export const importMappingPresetUpdateSchema = z
  .object({
...
const guidedHoldingSchema = z
  ...
  .superRefine((value, context) => {
    if (
      (value.marketValueAmount ?? 0) <= 0 &&
      !((value.quantity ?? 0) > 0 && (value.lastPriceAmount ?? 0) > 0)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["marketValueAmount"],
        message:
          "Each holding requires market value or quantity plus current price.",
      });
    }
  });

export const guidedImportCreateSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    if (value.accountMode === "existing" && !value.existingAccountId) {
      context.addIssue({
        ...
        message: "Select an existing account.",
      });
    }
    if (value.method === "manual-entry") {
      if ((value.holdings?.length ?? 0) === 0) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["holdings"],
          message: "Manual entry requires at least one holding.",
        });
      }
    }
  });
...
export const holdingEditSchema = z
  .object({
    name: z.string().trim().min(1).max(160).optional(),
...
export const holdingCreateSchema = z
  ...
  .refine(
    (value) =>
      value.marketValueAmount != null ||
      (value.quantity != null && value.lastPriceAmount != null),
...
export const accountMergePreviewSchema = z
  .object({
...
>;
export type LooMinisterSettingsInputPayload = z.infer<
  typeof looMinisterSettingsInputSchema
>;
export type DisplayLanguageInputPayload = z.infer<
  typeof displayLanguageInputSchema
>;
...
export type ImportMappingPresetCreatePayload = z.infer<
  typeof importMappingPresetCreateSchema
>;
export type ImportMappingPresetUpdatePayload = z.infer<
  typeof importMappingPresetUpdateSchema
>;
export type GuidedImportCreatePayload = z.infer<
  typeof guidedImportCreateSchema
>;
...
>;
export type CandidateScoreCreatePayload = z.infer<
  typeof candidateScoreCreateSchema
>;
export type CandidateCompareCreatePayload = z.infer<
  typeof candidateCompareCreateSchema
>;
export type GuidedAllocationDraftPayload = z.infer<
  typeof guidedAllocationDraftSchema
>;
export type CitizenOverrideInputPayload = z.infer<
  typeof citizenOverrideInputSchema
>;
export type HoldingSecurityTypePayload = z.infer<
  typeof holdingSecurityTypeSchema
>;
...
export type AccountMergePreviewPayload = z.infer<
  typeof accountMergePreviewSchema
>;
export type AccountMergeConfirmPayload = z.infer<
  typeof accountMergeConfirmSchema
>;
