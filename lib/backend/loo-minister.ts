...
const projectKnowledgeItems: Array<{
...
}> = [
  {
    ...
    value:
      "总览负责展示 Loo国当前全局状态：总资产、走势图、健康巡查、重点账户、头部持仓和今日秘闻。",
    detail:
  ...
  },
  {
    ...
    value:
      "组合页负责解释账户、持仓、资产类别分布、FX 折算、报价新鲜度和组合级健康状态。",
    detail:
  ...
  },
  {
    ...
    value:
      "投资偏好保留两条线：新手引导式问答生成完整参数，进阶用户手动编辑所有参数。",
    detail:
  ...
  },
  {
    ...
    value:
      "Health Score 分全组合和账户两个层级：全组合看总体配置/风险，账户页看账户内适配并参考全组合目标。",
    detail:
  ...
  },
  {
    ...
    triggers: [
      /约束|constraints|排除|preferred|excluded|偏好标的|账户规则|资产类别区间|security type/i,
    ],
    value:
      "推荐约束是 V2.1 的硬边界/软偏好层：排除标的、偏好标的、账户规则、资产类别区间和 security type 规则会影响推荐排序。",
    detail:
  ...
  },
  {
    ...
    pages: ["import"],
    triggers: [
      /导入|上贡|import|csv|新增账户|新增持仓|搜索标的|验证|券商同步|ibkr|interactive brokers|flex|token|授权口令|query id|查询编号/i,
    ],
    value:
      "移动端上贡页支持手动/引导式账户和持仓导入，并已接入 IBKR Flex 的预览草稿流程。",
    detail:
      "IBKR 导入需要两个值：Flex 网页服务生成的授权口令，以及活动 Flex 查询保存后的查询编号。Flex 查询应包含账户资料、当前持仓、现金报表和基础币种权益汇总。Loo国先生成草稿预览，不保存授权口令；用户确认后才写入账本。导入时标的搜索/验证必须显示 symbol、exchange/listing market、currency，避免 US common share 和 CAD listing/CDR 混淆。",
  },
  {
    ...
    triggers: [
      /新鲜|stale|缓存|刷新|provider|行情|fx|汇率|真实数据|mock|fallback/i,
    ],
    value:
      "行情、历史价格、FX 和外部情报都必须显示来源、新鲜度和 fallback 边界。",
    detail:
  ...
  },
  {
    ...
    value:
      "AI 大臣是跨页面 Loo国管家，用当前页面资料、用户偏好、推荐、缓存分析和对话上下文回答问题。",
    detail:
...
function formatChartFreshnessForUser(
  chartFreshness: string | null | undefined,
) {
...
function isImportSetupQuestion(question: string) {
  return (
    /导入|上贡|券商|ibkr|inkr|interactive brokers|flex|token|授权口令|query\s*id|查询编号|报表|同步|设置|账户/i.test(
      question,
    ) &&
    /怎么|如何|怎样|需要|设置|配置|拿到|获取|填写|填|新账户|token|授权口令|query\s*id|查询编号|flex|ibkr|inkr|券商/i.test(
      question,
    )
  );
}
...
function isUsableAssetClass(value: string | null | undefined) {
  ...
  return Boolean(
    normalized && !/未知|未确认|unknown|n\/a|--/i.test(normalized),
  );
}
...
function matchesSecurityIdentity(
  ...
  return (
    Boolean(symbol && exchange && currency) &&
...
function getTargetAllocationForAssetClass(
  ...
  return profile.targetAllocation.find(
    (target) => normalizeKey(target.assetClass) === normalizeKey(assetClass),
  )?.targetPct;
}
...
function mapPortfolioContextFacts(
...
function mapGlobalUserContextFacts(
  context: GlobalUserContext,
): LooMinisterFact[] {
      ...
      detail:
        context.topHoldings.length > 0
          ? `前几大持仓：${context.topHoldings
              .map(
                (holding) =>
                  `${holding.symbol} ${formatPct(holding.weightPct)}`,
              )
              .join("、")}`
          : undefined,
      source: "portfolio-data",
  ...
  if (security) {
    return {
      ...
      currentHolding: args.input.pageContext.subject.holdingId
        ? (args.holdings.find(
            (holding) =>
              holding.id === args.input.pageContext.subject.holdingId,
          ) ?? null)
        : null,
    };
  }
...
function buildSecurityContext(args: {
  ...
  if (
    ...
    throw new Error(
      "Security context requires a security or holding detail page.",
    );
  }
  ...
  if (!subject) {
    throw new Error(
      "Security context requires security identity or holdingId.",
    );
  }
  ...
  const assetClass = inferEconomicAssetClass({
    ...
    securityType:
      security.securityType ?? holdingsForContext[0]?.securityTypeOverride,
  ...
  const analysisFacts = facts.filter(
    (fact) => fact.source === "analysis-cache",
  );
  ...
  const provider =
    ...
    holdingsForContext.find((holding) => holding.quoteProvider)
      ?.quoteProvider ??
    null;
  const missing = [
  ...
  return {
    ...
    identity: {
      ...
      securityType:
        ...
        holdingsForContext[0]?.securityTypeOverride ??
        null,
    },
    holdingExposure: {
...
function buildCandidateFitContext(args: {
  ...
  const pageAssetClass = findFact(
    args.input.pageContext.facts,
    "asset-class",
  )?.value;
  const rawAssetClass =
...
function buildProductHelpAnswer(input: LooMinisterQuestionRequest) {
  const { pageContext, question } = input;
  if (pageContext.page === "import" && isImportSetupQuestion(question)) {
    return [
      "结论：如果你已经拿到 IBKR 授权口令，现在还需要确认“活动 Flex 查询”的查询编号，并确保查询里包含账户资料、当前持仓、现金报表和基础币种权益汇总。",
      "设置路径：在 IBKR 客户端门户进入 报告 → Flex 查询 → Flex 网页服务设置，启用服务后复制当前授权口令；再到 Flex 查询里新建活动 Flex 查询，保存后复制查询编号。授权口令不是账户号，查询编号也不是账户号。",
      "在 Loo国里：进入 上贡 → 券商同步 → IBKR Flex 预览，分别填入授权口令和查询编号，先读取预览。预览会显示账户、持仓和现金，确认后才写入账本；Loo国不会保存授权口令。",
      "需要特别确认：如果预览没有持仓，通常是活动 Flex 查询没有勾选“当前持仓”；如果某些持仓显示待确认，通常是 IBKR 只返回 SMART 路由或缺少实际上市交易所，暂不能直接写入主账本。",
      "边界：这一步只是在做导入设置和账本写入前检查，不需要先判断组合配置是否偏离或新账户是否适合加仓。",
    ].join("\n");
  }
  ...
  return [
    ...
    related.length > 0
      ? `相关功能边界：${related
          .map(
            (fact) => `${fact.label.replace("功能说明：", "")}：${fact.value}`,
          )
          .join("；")}`
...
function buildSecurityDetailAnswer(input: LooMinisterQuestionRequest) {
  const securityName = getSecurityDisplayName(
...
function buildAnalysisHandoffAnswer(input: LooMinisterQuestionRequest) {
  ...
  const scopeLine =
    actions.length > 0
      ...
      : "当前页面没有提供可直接运行的 Loo皇巡阅动作；请进入标的详情、Health Score、账户 Health 或推荐页后再触发。";

  return [
    "这类问题不应该只靠聊天回答。大臣会把它当作 Loo皇巡阅交接：先解释可分析范围，再让你确认是否运行后端受控分析。",
    scopeLine,
...
function buildAnswer(input: LooMinisterQuestionRequest) {
  ...
  if (
    ...
    findFact(pageContext.facts, "comparison-subject-1")
  ) {
  ...
  if (
    isProductHelpQuestion(question) &&
    hasProjectKnowledge(pageContext.facts)
  ) {
    return buildProductHelpAnswer(input);
  }
...
function hasUnavailableRunAnalysisPromise(answer: LooMinisterAnswerResult) {
  return /确认式\s*AI\s*快扫流程|确认式\s*Loo皇巡阅|由你确认后再运行|确认后再运行|点击确认.*快扫|点击确认.*巡阅|应用里.*确认.*快扫|应用里.*确认.*巡阅/i.test(
    answer.answer,
  );
}
...
function sanitizeUnavailableRunAnalysisPromise(
  ...
  if (
    hasRunAnalysisAction(input) ||
    !hasUnavailableRunAnalysisPromise(answer)
  ) {
    return attachDeterministicSuggestedActions(input, answer);
  }
  ...
  const cleanedAnswer = answer.answer
    ...
    .filter(
      (sentence) =>
        !/确认式\s*AI\s*快扫流程|确认式\s*Loo皇巡阅|由你确认后再运行|确认后再运行|点击确认.*快扫|点击确认.*巡阅|应用里.*确认.*快扫|应用里.*确认.*巡阅/i.test(
          sentence,
        ),
    )
    .join("")
    .trim();
  const boundary =
    "动作边界修正：本轮回答下方没有可确认的 Loo皇巡阅按钮，所以当前聊天只做轻量解释；如需 Loo皇巡阅，请进入标的详情、Health Score、账户 Health 或推荐页，使用页面里的巡阅卡片。";
  ...
  return attachDeterministicSuggestedActions(
    ...
    looMinisterAnswerResultSchema.parse({
      ...
      answer: [
        cleanedAnswer ||
          "GPT 主回答只包含不可执行的巡阅确认承诺，已移除该动作承诺。",
        boundary,
      ].join("\n\n"),
      keyPoints: [
        "本轮没有可确认的 Loo皇巡阅按钮；已保留 GPT 主回答并修正动作边界。",
        ...answer.keyPoints,
      ].slice(0, 8),
    }),
  );
...
function buildRouterCompatiblePrompt(input: LooMinisterQuestionRequest) {
  ...
  return [
    ...
    "如果用户要求帮忙分析、运行快扫、巡阅或生成分析报告，只有在本轮 allowedActions 里存在 run-analysis 动作时，才可以说页面会提供确认式 Loo皇巡阅按钮；否则必须说明当前聊天只能给轻量解释，并引导用户进入标的详情、Health Score、账户 Health 或推荐页运行对应巡阅。suggestedActions 必须返回 []，后端会基于 allowedActions 附加确认动作。",
  ...
  return [
    {
      role: "system",
      content:
        "你是 Loo国大臣，是项目内的投资管家型助手，不是数据字段解释器。只用提供的页面 context、用户持仓/偏好背景和缓存资料回答中文问题。默认按自然聊天方式回答，像 ChatGPT 一样直接回应用户问题；不要每次套用固定小标题、固定段落或报告模板。只有当用户明确要求分析报告、对比、清单或步骤时，才使用条列。回答要简洁，优先 1-3 段。不要把 DTO、overlay、deterministic、sourceMode、provider、fallback、run-analysis、context pack 等工程词直接说给用户。如果 context 里有 portfolio-context、security-context、candidate-fit、global-user-context、analysis-cache 或 external-intelligence 结果，优先引用它，但对用户要说“组合上下文、标的上下文、候选适配、用户持仓和偏好背景、缓存分析、外部资料”。上下文优先级必须是：当前页面事实和 security-context/candidate-fit > 用户明确提到的标的 > recentSubjects 补齐资料 > portfolio-context > global-user-context > project knowledge。global-user-context 是用户级背景，只能补足持仓/偏好/推荐背景，不能覆盖当前页面或当前标的事实。没有资料时说明只能基于页面上下文和本地缓存回答。不要编造实时行情、新闻或论坛结论；保留 securityId 以及 symbol + exchange + currency 身份；投资相关回答必须保留不构成投资建议口径。若用户要求帮忙分析、运行快扫、巡阅或生成分析报告，只有当 pageContext.allowedActions 里存在 run-analysis 时，才可以说页面会提供确认式 Loo皇巡阅按钮；否则说明当前聊天只能轻量解释，并引导用户进入对应页面运行巡阅。suggestedActions 必须返回 []，后端会基于 pageContext.allowedActions 附加确认动作。若用户问整体持仓、组合、配置、Health 或推荐，优先解释组合上下文；若缺少 portfolio-context 但存在 global-user-context，也必须使用用户持仓和偏好背景回答。若用户问标的/持仓详情，优先解释标的上下文；若用户问某标的是否适合买入/适配，进一步解释候选适配资料，并把标的本身、底层经济暴露、现有持仓重复度、目标配置缺口、Preference Factors、账户/税务/FX、数据新鲜度都纳入；currentExposure=0 只代表未持有，不得阻止 candidate-new-buy 分析。只返回合法 JSON object，不要 markdown。JSON 必须符合 LooMinisterAnswerResult：version、generatedAt、role、page、title、answer、structured、keyPoints、suggestedActions、sources、disclaimer。answer 是面向用户展示的聊天正文，不能机械拼接“判断依据/需要确认/边界/下一步”等固定标题。structured 只作兼容字段：directAnswer 填 answer 的一句话摘要；reasoning、decisionGates 填 []；boundary 和 nextStep 默认 null，除非用户明确要求严肃投资分析或操作步骤。role 必须是 loo-minister，产品动作由本地应用控制。",
    },
    {
      role: "user",
...
async function enrichMinisterInputWithDailyIntelligence(
  ...
  try {
    ...
    const matchedItems = subjectListingKey
      ...
      : [];
    const selectedItems = (
      matchedItems.length > 0 ? matchedItems : items
    ).slice(0, 3);
    ...
    const holdings = await getOrBuildContextPack({
      key: securityContextPackKey({
        ...
        identity: [
          ...
          .join("|"),
        quoteUpdatedAt:
          input.pageContext.dataFreshness.quotesAsOf ?? input.pageContext.asOf,
      }),
      kind: "security",
