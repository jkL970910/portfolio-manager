...
| 5    | P0.5 Security Research Cockpit UI  | Completed | Security detail now opens with a research-workbench frame and auto-runs local deterministic quick scan so the top flow shows conclusion, guardrails, fit, next steps, evidence, and optional user-triggered GPT enhancement. |
...
| 8    | P0.8 Manual QA SOP                 | Completed | `docs/guides/mobile-manual-qa-sop.md` now includes a dedicated P0.1-P0.7 decision-chain QA section for DTO, guardrails, portfolio fit, research cockpit, evidence/freshness, GPT boundary, and Minister action boundary. |
...
| Quote / FX / history freshness | Mostly Completed | Quote refresh preserves native trading currency, FX is cached separately for display conversion, history is stored by listing identity, charts expose freshness/source/fallback state, provider retry-after is persisted, and Settings shows provider status. | Complete scheduled history depth and deeper per-provider dashboards; keep avoiding page-load live calls. |
...
| AI quick-scan readability / exposure split | In Progress | Security quick scan now separates listing identity from economic exposure through shared `security-economic-exposure.ts`, used by 智能快扫, Recommendation V2.1, Health Score, and 大臣 context. CAD-listed US ETFs stay `US Equity` economically; CGL.C / gold / precious-metals instruments are treated as `Commodity` / 商品贵金属 exposure instead of Canadian Equity solely because they are CAD/TSX-listed. Mobile result layout is now scope-aware: security scans use `投资判断 / 买入前确认 / 风险护栏 / 组合适配 / 数据依据 / 来源详情`, while portfolio/account/recommendation scans use diagnostic labels. The opening thesis is decision-first and uses target sleeve gap, Preference Factors V2, account/tax/FX path, holding concentration, and data freshness before showing scorecards. Avoided sectors, low risk capacity, high-priority home purchase goals, thin/stale data, duplicate exposure, and USD/account/tax friction are treated as blockers before any buy/add wording. Debug-like market-data strings stay out of the opening thesis and dense evidence is folded under `数据依据`. Security-specific `securityDecision` now gives explicit lens/verdict/direct answer/why-now/blockers/watch triggers/evidence, while legacy scorecards remain supporting data for compatibility. Default quick scans are product-owned deterministic smart scans; external GPT is now an explicit `GPT 增强解读` action that reuses Loo Minister GPT settings and keeps the base result visible on provider failure. |
