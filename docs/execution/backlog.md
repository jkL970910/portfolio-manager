...
| AI quick-scan readability / exposure split | In Progress | Security quick scan now separates listing identity from economic exposure for CAD-listed US ETFs through shared `security-economic-exposure.ts`, used by AI 快扫, Recommendation V2.1, and Health Score. Mobile AI result layout is now `核心结论 / 当前分析结论 / 风险护栏 / 组合适配 / 数据依据 / 来源详情`, with debug-like market-data strings kept out of the opening thesis. |
