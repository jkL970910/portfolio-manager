...
| Portfolio analyzer / cached external intelligence | Partially Completed | `portfolio-analyzer.skill` is productized as cached/guarded analysis. Cached market-data external research persists structured `external_research_documents`; Recommendation V3 overlay and Loo国今日秘闻 can consume persisted/cached documents. | Prove more end-to-end QA on real cached data and saved analysis runs before enabling live external research adapters. |
| Live external information providers | Not Completed | Raw news/forum/announcement/live external research adapters remain disabled. Current system has contracts, TTL, quota, job, and document storage boundaries only. | Next P0: add one bounded, cache-first, structured external-information adapter. Prefer announcements/filings/fundamental profile/earnings-calendar style data before raw news/forum/search. |
...
1. Add the first live external-information adapter behind the existing worker/cache/quota boundary. Prefer structured, low-frequency data such as filings/announcements/earnings calendar/fundamental events before raw news/forum/search. Output must persist to `external_research_documents` with identity scope, TTL, confidence, relevance, reliability, and risk flags.
2. Run end-to-end QA for `portfolio-analyzer.skill`, AI 标的快扫, AI 大臣, Loo国今日秘闻, and Recommendation V3 overlay on cached real data. The goal is to prove the pipeline consumes persisted provider data without live page-load calls.
