...
| Loo国 AI Minister assistant               | In Progress | Backend and Flutter first-pass page-context DTOs exist; Overview and Portfolio now build minister context with metrics, FX, chart freshness, quote status, allocation/drift, and safe actions; next is guarded question API                                       |
...
3. Add the guarded Loo Minister question API, then wire the first user-triggered mobile entry point.
