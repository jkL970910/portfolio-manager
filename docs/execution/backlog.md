...
| 10   | P0.10 Research Cockpit Plan Lock  | Completed | Architecture locked in `docs/execution/security-research-cockpit.md`: valuation is evidence with confidence, not an automated DCF oracle; ETF logic uses macro/rebalancing framing; backend rules own conclusions and GPT/大臣 explain only. |
...
| Security identity and metadata                    | Completed for current P0              | Canonical `security_id` is the join key for holdings, history, recommendations, analysis cache, and external intelligence. Metadata has project-registry, OpenFIGI identity/alias support, Alpha Vantage profile provider, exact `SYMBOL:EXCHANGE:CURRENCY` QA allowlists, `force=true` QA refresh, manual locks, provider confidence guards, and a shared `SecurityExposureProfile` layer that separates listing identity from economic exposure. CAD-listed US ETFs/CDRs such as ZQQ/VFV stay CAD/TSX for quote routing, but portfolio allocation and fit use underlying US Equity exposure; CGL.C/gold instruments use Commodity / Precious Metals exposure instead of Canadian Equity solely because they trade in CAD. | Broaden provider QA from representative holdings to watchlist/non-held candidates; expand the project registry/provider mapping for more ETF sector/region exposures only when real holdings or user tests reveal gaps. |
...
| Portfolio analyzer / cached external intelligence | Mostly Completed                      | `portfolio-analyzer.skill` is productized as cached/guarded analysis. Cached market-data external research, Alpha Vantage profile research, and Alpha Vantage institutional earnings research can persist structured `external_research_documents`; `market_sentiment_snapshots` now stores first-pass US market pulse data with FGI, VIX, and an A-I 3x3 matrix strategy; `Loo国今日秘闻`, Recommendation V3 overlay, and AI 大臣 have automated coverage proving they consume persisted/cached context without page-load live calls. Daily overview intelligence now has a worker-only enqueue path that samples a small set of complete-identity holdings, skips fresh cached documents, and then drains the same worker queue. | Run cloud smoke for the new institutional source, then evaluate stable provider-backed FGI/VIX sources before raw news/forum adapters.                 |
| Live external information providers               | News Adapter Added                    | The first bounded live adapter is `profile`, backed by Alpha Vantage `OVERVIEW` / `ETF_PROFILE`. The second bounded adapter is `institutional`, backed by Alpha Vantage `EARNINGS`, for company-style earnings context. The third bounded adapter is `news`, backed by Alpha Vantage `NEWS_SENTIMENT`; it is intended for worker-only daily overview `Loo国今日秘闻` cache writes, not page-load live calls or single-security automatic news refresh. All three run through the external-research worker/cache/quota boundary with `security_id` and listing identity preserved where applicable. Forum/community adapters remain disabled.                                                                                 | Cloud-smoke `source=news` with a small daily batch, verify Overview shows cached news cards, and keep user-facing quota copy source-specific.          |
...
| Recommendation V3 / 进货工作台             | In Progress               | See `docs/execution/recommendation-v3-external-intelligence.md`. The current UI foundation exists; next backend slice is `CandidatePoolPolicy` + dynamic candidate pool governance. V3 remains deterministic and rules-first: target gaps, account/tax/FX placement, economic exposure, concentration, cash/home-goal guardrails, and explicit user exclusions own the decision. Cached intelligence, market pulse, AI 大臣, and future Daily Briefs are evidence/explanation only and must not override identity or allocation rules. If hard filters leave zero eligible candidates, the API must return `needs_policy_relaxation` with explicit blockers and suggested relaxations; it must not silently force a default core ETF. |
| Loo国今日秘闻                              | In Progress               | Standalone mobile API now combines persisted external research documents and saved analysis runs into a curated feed. Flutter Overview is the only page with the full daily briefing card. Recommendations no longer loads or renders the full card; it only shows whether cached external material was incorporated and keeps item-level `相关秘闻` evidence on recommendation cards. Portfolio no longer loads the feed, and identity-filtered Security Detail keeps the strict listing-level card. Daily overview refresh runs only through worker/cache/quota boundaries. Single-security manual refresh now lives under the Security Detail `研究资料状态 / 更新` sheet, where quote/history refresh, profile refresh, institutional refresh, and local regeneration stay separate while sharing one UI entry. The sheet exposes daily quota, cache TTL, latest task state, and source-specific status matched by full security identity and source id.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 今日市场脉搏 / VIX + FGI context           | In Progress               | `market_sentiment_snapshots` now supports worker/cache-only market pulse refresh. VIX is provider-backed through cached `^VIX` quote/history. FGI first attempts a daily cached CNN Fear & Greed GraphData fetch through the protected `/api/workers/market-sentiment/run` endpoint; this is deterministic HTTP parsing, not an LLM call, so it consumes no GPT tokens. The previous placeholder `信用风险偏好 / 避险需求` components are removed from the user-facing market pulse. The worker now attempts real FRED macro indicators for `信用压力` (`BAMLH0A0HYM2`) and `利率压力` (`DGS10`), plus detail-page indicators such as real rate (`DFII10`), yield curve (`T10Y2Y`), HYG/LQD/TLT/GLD/UUP where provider data is available. If a data source fails, the UI omits or marks it unavailable instead of fabricating a value. Overview displays VIX, CNN FGI/Loo 情绪分, A-I matrix strategy, major-index performance, and scrollable real-indicator details as low-weight timing/risk context. Page load must only read cached snapshots; live CNN/VIX/FRED/ETF fetches belong to worker/manual protected refresh paths.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
1. `P1.R1 CandidateProvider abstraction`
   - Wrap the current static `Core ETF Universe` behind a provider interface.
   - Preserve existing V2.1 recommendation fields and current Flutter contract.
   - Add tests proving current core-pool output does not change.
2. `P1.R2 CandidatePoolPolicy hard filter`
   - Build policy from Preference Factors V2, recommendation constraints,
     account availability, and portfolio cash/liability context.
   - Apply policy before scoring.
   - Treat explicit exclusions and `excludeRoles` as hard filters.
3. `P1.R3 Empty-pool contract`
   - Add `needs_policy_relaxation` status when hard filters remove every
     candidate.
   - Return blocker list and suggested relaxations.
   - Do not silently insert `CASH.TO`, `VFV.TO`, or any other default symbol.
4. `P1.R4 User-confirmed fallback`
   - Add optional `fallbackMode: "core_only_relaxed"`.
   - Only run after explicit user confirmation.
   - Restrict fallback to high-confidence core-pool candidates and label the
     result as relaxed-rule output.
5. `P1.R5 Dynamic candidate registry`
   - Add a DB-backed registry for provider-discovered candidates.
   - Store clean identity, economic exposure, role, expense/liquidity, provider
     confidence, source, TTL, and eligibility notes.
   - Low-confidence candidates go to `待鉴定包裹`, not the primary ranking.
6. `P1.R6 Worker ingestion`
   - Refresh only held securities, watchlist entries, and core-pool candidates
     first.
   - Upsert dynamic candidates only when identity/profile quality is acceptable.
   - Keep all provider calls in worker/cache/quota paths; no page-load provider
     discovery.
7. `P1.R7 Mobile policy UI`
   - Show empty-pool state as `进货规矩过严，暂无可推荐标的`.
   - Offer `放宽核心池`, `去设置调整规则`, and `查看被排除原因`.
   - Show source badges: `核心池`, `囤货清单`, `动态发现`, `放宽规则`.
8. `P1.R8 Recommendation QA`
   - Test aggressive/high-risk profile where routine cash is excluded.
   - Test liability/home-goal profile where cash re-enters despite high risk.
   - Test over-strict exclusions producing `needs_policy_relaxation`.
   - Test explicit fallback and verify no fallback happens silently.
...
1. Finish and QA the active Overview UI v2 pass. The current design pass should
   remove obvious layout/wording problems first, then commit/deploy before
   starting another large backend feature.
2. Lock P0.10 Security Research Cockpit architecture. Use
   `docs/execution/security-research-cockpit.md` as the implementation source:
   backend rules decide, valuation is evidence, ETF uses macro/rebalancing
   framing, and GPT/大臣 explain only.
3. Complete the remaining QA/review for `P1.1 SecurityResearchDecision DTO`.
   First pass is implemented as an additive `securityResearchDecision` result
   block with stock/ETF fixtures and legacy quick-scan compatibility.
4. Expand `P1.2 Valuation Evidence MVP` after the first cached Alpha Vantage
   profile pass: add historical multiple percentile / second-provider sanity
   checks later, but do not build full automated DCF until the DTO and mobile
   rendering are stable.
5. Extend `P1.2b ETF Macro Proxy` later with deeper ETF data such as holdings,
...
Status: active next implementation after Figma direction signoff.
...
1. Flutter theme foundation: `Rose Treasury` dark tokens, `Rose Day` light
   tokens, semantic colors, typography scale, spacing, radii, and theme mode
   setting (`system` / `light` / `dark`). First pass implemented on
   2026-05-08; Settings toggle still pending.
2. Reusable component layer: glass cards, hero header, metric cards, decision
   cards, chart cards, key-value tables, clean tappable rows, health radar, AI
   scan card, minister chat bubble, forms, bottom sheets, and empty/loading/error
   states. First pass implemented for theme, glass cards, hero, metric cards,
   tappable rows, key-value table, and state panel.
3. Overview: premium first fold, total asset chart, complete account entry
   coverage, concise daily intelligence, and no duplicate technical FX cards.
   First pass implemented on 2026-05-08: v2 foundation applied, account/holding
   row arrows removed, full returned account/top-holding lists are shown.
4. Portfolio: complete account and holding access, no artificial `take(3)` /
   `take(12)` style hidden lists, dense tables/rows, and detail navigation from
   every visible row. First pass implemented on 2026-05-08: v2 foundation
   applied, row arrows removed, and the page shows the full returned
   account/holding lists.
5. Security Detail / Loo国研究台: cockpit hero, decision card, chart, compact
   evidence sections, optional AI/GPT enhancement, and strict listing identity.
6. Account Detail, Recommendations, Import, Preferences, Minister, Settings,
   and Health: migrate page by page without losing current behavior.
7. Manual QA pass and docs update after each major page migration.

Non-negotiable UI rules:

- Do not hide unimplemented data access issues behind visual polish.
- Do not show debug/internal terms in primary user cards.
- Do not use list badges such as `已更新` / `未持有` when they compete with the
  main content; move those details to subtle metadata or detail pages.
- Do not use small arrow icons as the only detail affordance; the whole row/card
  should be tappable.
- Do not duplicate full `Loo国今日秘闻`, FX policy, or provider status cards
  across multiple top-level pages.
