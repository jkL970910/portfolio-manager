...
| Portfolio analyzer / cached external intelligence | Mostly Completed | `portfolio-analyzer.skill` is productized as cached/guarded analysis. Cached market-data external research and Alpha Vantage profile research can persist structured `external_research_documents`; `Loo国今日秘闻`, Recommendation V3 overlay, and AI 大臣 have automated coverage proving they consume those persisted documents without page-load live calls. | Run real local/cloud smoke with profile documents, then add one bounded announcement/filing/earnings-calendar adapter before raw news/forum adapters. |
| Live external information providers | First Structured Adapter Implemented | Raw news/forum/announcement adapters remain disabled. The first bounded live adapter is `profile`, backed by Alpha Vantage `OVERVIEW` / `ETF_PROFILE`, and only runs through external-research worker/cache/quota boundaries. Profile smoke tooling now supports source-specific QA with `security_id`, security type, source flags, and Alpha Vantage key checks. | Next P0: run real profile smoke on representative held/watchlist candidates, then add one announcement/filing/earnings-calendar style adapter before any raw news/forum/search. |
...
1. Run real local/cloud profile smoke on representative held/watchlist candidates: enqueue a profile-enabled job, run the worker, confirm `external_research_documents`, `Loo国今日秘闻`, Recommendation V3 overlay, and AI 大臣 consume the persisted document without page-load live calls.
...
3. Broaden security metadata provider QA from representative holdings to watchlist and non-held candidates. Keep exact `SYMBOL:EXCHANGE:CURRENCY` identities and never reintroduce ticker-only joins.
4. Finish production cloud validation: Vercel env, Cloudflare cron secret, worker smoke runs, and mobile Settings status confirmation for market-data, security-metadata, external-research, and provider usage.
5. Complete scheduled history depth and per-provider status polish so quote/history/FX freshness is explainable without debug jargon.
6. Extend Preference Factors V2 with AI 大臣问答式 parameter generation using the same payload as the manual Flutter editor.
7. Continue backend contract typing for detail pages and AI context DTOs so mobile stops relying on page-level `Map<String, dynamic>` parsing.
8. Harden mobile auth with revocable refresh tokens and production storage policy.
9. Keep Mobile UI / IA overhaul in P1 until the data/AI layer is credible.
10. Migrate spending/cash account monitoring into a dedicated mobile flow after investment/AI foundations are stable.
