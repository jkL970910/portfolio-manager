...
| Portfolio analyzer / cached external intelligence | Partially Completed | `portfolio-analyzer.skill` is productized as cached/guarded analysis. Cached market-data external research and Alpha Vantage profile research can persist structured `external_research_documents`; Recommendation V3 overlay and Loo国今日秘闻 can consume persisted/cached documents. | Prove more end-to-end QA on real cached data, profile documents, and saved analysis runs before enabling raw news/forum adapters. |
| Live external information providers | First Structured Adapter Implemented | Raw news/forum/announcement adapters remain disabled. The first bounded live adapter is `profile`, backed by Alpha Vantage `OVERVIEW` / `ETF_PROFILE`, and only runs through external-research worker/cache/quota boundaries. | Next P0: QA profile documents end-to-end, then add one announcement/filing/earnings-calendar style adapter before any raw news/forum/search. |
...
1. QA the new Alpha Vantage profile external-research adapter end to end: enqueue a profile-enabled job, run the worker, confirm `external_research_documents`, `Loo国今日秘闻`, Recommendation V3 overlay, and AI 大臣 consume the persisted document without page-load live calls.
