...
- FX rates now have an independent `fx_rates` store. Manual quote/security
  refresh checks stored USD/CAD freshness and only calls the FX provider when
  missing/stale, then uses the latest stored FX or a conservative fallback for
  CAD aggregation. Settings also exposes an explicit `刷新 FX 汇率` force-refresh
  action.
