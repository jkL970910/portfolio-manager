...
| HoldingTable | stable | [holding-table.tsx](E:\Projects\Portfolio%20Manager\components\portfolio\holding-table.tsx) | Shared holdings detail table with optional deep-link highlight state, explicit separate entry pills for `持仓详情 / Holding detail` and `标的资料 / Security page`, explicit total-shares and average-cost display, a clear split between share of total portfolio and share inside the current account, shorter Chinese headers, a compressed current-value column, clean UTF-8 copy, and Loo皇-style review copy in Chinese mode |
