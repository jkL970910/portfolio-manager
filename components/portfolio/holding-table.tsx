      ...
      <table className="min-w-full text-left text-sm">
        <thead className="text-[color:var(--muted-foreground)]">
          <tr>
            ...
            <th className="pb-3 font-medium">{pick(language, "总股数和平均成本", "Shares and average cost")}</th>
            <th className="pb-3 font-medium">{pick(language, "这笔现在值多少", "Current value")}</th>
            ...
            >
              <td className="py-4 font-medium text-[color:var(--foreground)]">
                {holding.href ? (
                  ...
                  <div className="flex items-start gap-3 px-2 py-2">
                    <SecurityMark symbol={holding.symbol} assetClass={holding.assetClass} />
                    <div className="flex flex-col gap-2">
                      <span className="font-semibold text-[color:var(--foreground)]">{holding.symbol}</span>
                      <span className="text-xs font-medium text-[color:var(--muted-foreground)]">{holding.name}</span>
                      {holding.highlightLabel ? (
                        <span className="inline-flex w-fit rounded-full border border-[rgba(232,121,249,0.22)] bg-[rgba(255,255,255,0.82)] px-2.5 py-1 text-xs font-medium text-[color:var(--foreground)]">
                          {holding.highlightLabel}
              ...
              <td className="py-4">
                <div className="space-y-1 text-sm">
                  <div>
                    <span className="text-[color:var(--muted-foreground)]">{pick(language, "总股数", "Total shares")}</span>
                    <span className="ml-2 font-medium text-[color:var(--foreground)]">{holding.quantity}</span>
                  </div>
                  <div>
                    <span className="text-[color:var(--muted-foreground)]">{pick(language, "平均成本", "Average cost")}</span>
                    <span className="ml-2 font-medium text-[color:var(--foreground)]">{holding.avgCost}</span>
                  </div>
                </div>
              </td>
              <td className="py-4 font-medium text-[color:var(--foreground)]">{holding.value}</td>
